...
const singleTypeEffectiveness = (attack: Type) => (
  defenseType: Type
): Effectiveness => {
  const doubles: Effectiveness[] = defenseType.doubleDamageFrom.flatMap(
    (effectiveName) => {
      return effectiveName === attack.name ? [Double] : [];
    }
  );

  const half: Effectiveness[] = defenseType.halfDamageFrom.flatMap(
    (effectiveName) => {
      return effectiveName === attack.name ? [Half] : [];
    }
  );

  const immune: Effectiveness[] = defenseType.noDamageFrom.flatMap(
    (effectiveName) => {
...
export function attackEffectiveness(
  attack: Type,
  defenseTypes: Type[]
): Effectiveness {
  const individualEffectivenesses: Effectiveness[] = defenseTypes.map(
    singleTypeEffectiveness(attack)
  );
